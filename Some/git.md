...
`git pull`
Если локальные файлы отличаются от репозитория:
`git checkout .` или в новом git `git restore .` - отменить все изменения
`git checkout -- requirements.txt` — отменить изменения в одном файле.
